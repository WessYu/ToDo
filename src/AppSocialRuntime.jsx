...
function App() {
  ...
  async function askAi() {
    ...
    setAiAnswer('');
    try {
      const payload = await apiRequest('/api/ai', { method: 'POST', body: JSON.stringify({ prompt: aiPrompt || 'Analise minhas tarefas compartilhadas, GitHub, Apple e presenca mensal.', date: selectedDate, tasks: selectedTasks, friends: data.friends, presence: { monthCompleted, monthPlanned, monthGithub, activeDays } }) }, token);
  ...
  return (
    <div className="neo-app">
      ...
      <main className="neo-workspace">
        ...
        {view === 'perfil' && (
          <section className="profile-layout"><div className="terminal-frame profile-card"><div className="profile-avatar">{user.avatar || 'R'}</div><span className="kicker">ACTIVE USER</span><h2>{user.name}</h2><p>@{user.username}</p><p>{user.email}</p><p>{data.apple.email ? `Apple: ${data.apple.email}` : 'Apple ainda nao conectado'}</p><div className="metric-strip mini"><div className="metric-card"><span>TASKS</span><strong>{data.tasks.length}</strong></div><div className="metric-card"><span>FIXAS</span><strong>{data.fixedTasks.length}</strong></div><div className="metric-card"><span>DONE</span><strong>{totalCompleted}</strong></div><div className="metric-card"><span>AMIGOS</span><strong>{data.friends.length}</strong></div></div></div><form className="terminal-frame neo-form profile-form" onSubmit={saveProfile}><span className="kicker">USER PROFILE</span><h2>Editar perfil</h2><label>Nome<input value={profileDraft.name} onChange={(event) => setProfileDraft((current) => ({ ...current, name: event.target.value }))} /></label><label>Usuario<input value={profileDraft.username} onChange={(event) => setProfileDraft((current) => ({ ...current, username: event.target.value }))} /></label><label>GitHub username<input value={profileDraft.githubUsername} onChange={(event) => setProfileDraft((current) => ({ ...current, githubUsername: event.target.value }))} placeholder="WessYu" /></label><label>Avatar<input value={profileDraft.avatar} onChange={(event) => setProfileDraft((current) => ({ ...current, avatar: event.target.value }))} maxLength={2} /></label><label>Bio<textarea value={profileDraft.bio} onChange={(event) => setProfileDraft((current) => ({ ...current, bio: event.target.value }))} placeholder="Foco, estudo, dev, design..." /></label><button className="neo-primary" type="submit">SALVAR PERFIL</button><button className="outline-button" type="button" onClick={syncGithub} disabled={githubSyncing}>{githubSyncing ? 'SINCRONIZANDO...' : 'VINCULAR / SINCRONIZAR GITHUB'}</button></form><section className="terminal-frame neo-form profile-form"><span className="kicker">APPLE ACCOUNT</span><h2>Sincronizar Apple</h2><p>Salva sua conta Apple aqui. Sync real com iCloud/Calendario precisa de Sign in with Apple e configuracao no backend.</p><label>Email Apple<input value={profileDraft.appleEmail} onChange={(event) => setProfileDraft((current) => ({ ...current, appleEmail: event.target.value }))} placeholder="seuemail@icloud.com" /></label><button className="neo-primary" type="button" onClick={connectApple}>CONECTAR APPLE</button><div className="sync-status-card"><span>{data.apple.email ? 'Conectado' : 'Nao conectado'}</span><strong>{data.apple.email || 'Sem conta Apple'}</strong>{data.apple.connectedAt && <p>Salvo em {new Date(data.apple.connectedAt).toLocaleString('pt-BR')}</p>}</div></section><section className="terminal-frame profile-form friends-panel"><span className="kicker">SOCIAL TASKS</span><h2>Amigos</h2><p>Adicione sua namorada ou amigos para criar tarefas compartilhadas.</p><form className="neo-form friend-form" onSubmit={addFriend}><label>Nome<input value={friendDraft.name} onChange={(event) => setFriendDraft((current) => ({ ...current, name: event.target.value }))} placeholder="Nome da pessoa" /></label><label>Email<input value={friendDraft.email} onChange={(event) => setFriendDraft((current) => ({ ...current, email: event.target.value }))} placeholder="Opcional" /></label><label>Avatar<input value={friendDraft.avatar} onChange={(event) => setFriendDraft((current) => ({ ...current, avatar: event.target.value }))} maxLength={2} placeholder="A" /></label><button className="neo-primary" type="submit">ADICIONAR AMIGO</button></form><div className="friend-list">{data.friends.map((friend) => <article className="friend-card" key={friend.id}><span>{friend.avatar || friend.name.slice(0, 1)}</span><div><strong>{friend.name}</strong><small>{friend.email || 'sem email'}</small></div><button type="button" onClick={() => removeFriend(friend.id)}>X</button></article>)}{!data.friends.length && <EmptyTerminal title="Sem amigos ainda" text="Adicione uma pessoa para criar tarefas compartilhadas." />}</div></section></section>
        )}
